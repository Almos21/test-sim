...
async function main() {
  ...
  const perfMap = {
    ...
    'Digit3': { name: 'VIENTO', toggle: params.windEnabled, keys: { 'KeyQ': { label: 'Viento X', target: params.wind, vector: 'x', step: 5.0, min: -10, max: 10 }, 'KeyW': { label: 'Viento Y', target: params.wind, vector: 'y', step: 5.0, min: -10, max: 10 } } },
    'Digit4': { name: 'ONDAS BPM', toggle: params.waveEnabled, keys: { 'KeyQ': { label: 'Fuerza', target: params.waveStrength, step: 15.0, min: -20, max: 20 }, 'KeyW': { label: 'Frecuencia', target: params.waveFrequency, step: 2.0, min: 0.1, max: 10 }, 'KeyE': { label: 'BPM', target: params.waveBPM, step: 40.0, min: 30, max: 240 }, 'KeyR': { label: 'Radio', target: params.waveRadius, step: 6.0, min: 0.5, max: 15 } } },
    'Digit5': { name: 'CAÑÓN (Dispara con Clic)', toggle: params.beamEnabled, keys: { 'KeyQ': { label: 'Poder', target: params.beamStrength, step: 150.0, min: 10, max: 500 }, 'KeyW': { label: 'Grosor', target: params.beamRadius, step: 1.0, min: 0.05, max: 3.0 } } },
    'Digit6': { name: 'GLOBAL (Entorno)', toggle: params.dragEnabled, keys: { 'KeyQ': { label: 'TimeScale', target: params.timeScale, step: 1.0, min: 0.0, max: 3.0 }, 'KeyW': { label: 'Fricción (Drag)', target: params.dragCoefficient, step: 0.5, min: 0.0, max: 1.0 }, 'KeyE': { label: 'Max Speed', target: params.maxSpeed, step: 15.0, min: 0.1, max: 100.0 }, 'KeyR': { label: 'Tamaño Partícula', target: params.particleSize, step: 0.05, min: 0.001, max: 0.1 } } },
    'Digit8': { name: 'CAOS BINARIO', toggle: params.binaryEnabled, keys: { 'KeyQ': { label: 'Gravedad', target: params.binaryGravity, step: 5.0, min: 0.1, max: 20.0 }, 'KeyW': { label: 'Prob. Choque %', target: params.repulsionChance, step: 2.0, min: 0.0, max: 10.0 }, 'KeyE': { label: 'Violencia (Repulsión)', target: params.repulsionStrength, step: 200.0, min: 0.0, max: 800.0 } } }
  };
  ...
  function updateHUD() {
    let html = `<div style="margin-bottom:10px; color:#aaa;"><strong>PERFORMANCE MODE</strong> | [P]: Ocultar/Mostrar Menú | [Space]: Reset</div>`;
